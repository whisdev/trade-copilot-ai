...
A dashboard for juggling multiple chat contexts (Twitter, Reddit, Discord, different channel types, usernames). Uses **OpenAI** for chat and for scoring how attractive each reply is for collaboration. Everything gets cached in the DB.
...
- **Backend:** Python FastAPI, MongoDB, OpenAI for chat and scoring.
...
3. **Send a message** — Type something and hit Send. The backend uses your cached history + OpenAI to reply and compute an **attract score** (0–100). You’ll see the score in the top right and on the assistant’s message.
